...
ifstream infile("argument.txt");
//ofstream outfile("argument.txt", ios::app);
...
BEGIN_MESSAGE_MAP(CPHZNVisionDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON8, &CPHZNVisionDlg::OnBnClickedButton8)
	ON_BN_CLICKED(IDC_BUTTON9, &CPHZNVisionDlg::OnBnClickedButton9)
END_MESSAGE_MAP()
...
{
	// TODO:  在此添加控件通知处理程序代码
	//outfile << delay << "  " << exp << "  " << rc << "  " << hv_Row11.D() << "  " << hv_Column11.D() << "  " << hv_Row12.D() << "  " << hv_Column12.D() << endl;
	//outfile << delay1 << "  " << exp1 << "  " << rc1 << "  " << hv_Row21.D() << "  " << hv_Column21.D() << "  " << hv_Row22.D() << "  " << hv_Column22.D() << endl;
...
UINT CPHZNVisionDlg::StartCameraTest(LPVOID pParam)
{
	...
	while (pDlg->StartImageState)
	{
		...
		if (pDlg->repaint.GetCheck() != 1 && judg1 == true && arg1 == false)
			continue;
...
UINT CPHZNVisionDlg::StartCameraTest1(LPVOID pParam)
{
	...
	while (pDlg->StartImageState1)
	{
		...
		if (pDlg->repaint1.GetCheck() != 1 && judg2 == true && arg2 == false)
			continue;
		//Mat srcImage = HObject2Mat(pDlg->ho_Image);
		/*if (judg2 == true)
...
void CPHZNVisionDlg::OnBnClickedButton2()
{
	...
	if (StartImageState == false && StartImageState1 == false)
	{
...
{
	// TODO:  在此添加控件通知处理程序代码
	//ofstream outfile("argument.txt", ios::trunc);
...
double hv_Row11d, hv_Column11d, hv_Row12d, hv_Column12d, hv_Row21d, hv_Column21d, hv_Row22d, hv_Column22d;
void CPHZNVisionDlg::OnBnClickedButton8()
{
	// TODO:  在此添加控件通知处理程序代码
	ofstream outfile("argument.txt", ios::trunc);
	//ofstream outfile("argument.txt", ios::app);
	if (thread1 == true)
	{
		hv_Row11d = hv_Row11.D();
		hv_Column11d = hv_Column11.D();
		hv_Row12d = hv_Row12.D();
		hv_Column12d = hv_Column12.D();
		outfile << delay << "  " << exp << "  " << rc << "  " << hv_Row11d << "  " << hv_Column11d << "  " << hv_Row12d << "  " << hv_Column12d << endl;
	}
	if (thread2 == true)
	{
		hv_Row21d = hv_Row21.D();
		hv_Column21d = hv_Column21.D();
		hv_Row22d = hv_Row22.D();
		hv_Column22d = hv_Column22.D();
		outfile << delay1 << "  " << exp1 << "  " << rc1 << "  " << hv_Row21d << "  " << hv_Column21d << "  " << hv_Row22d << "  " << hv_Column22d << endl;
	}
}


void CPHZNVisionDlg::OnBnClickedButton9()
{
	// TODO:  在此添加控件通知处理程序代码
	arg1 = arg2 = true;
	/*infile >> delay;
	...
	infile >> rc;
	infile >> hv_Row11d;
	hv_Row11 = HTuple(hv_Row11d);
	infile >> hv_Column11d;
	hv_Column11 = HTuple(hv_Column11d);
	infile >> hv_Row12d;
	hv_Row12 = HTuple(hv_Row12d);
	infile >> hv_Column12d;
	hv_Column12 = HTuple(hv_Column12d);
	//}
	/*infile >> delay1;
	infile >> exp1;
	infile >> rc1;*/
	...
	try
	{
		...
		infile >> rc1;
		infile >> hv_Row21d;
		hv_Row21 = HTuple(hv_Row21d);
		infile >> hv_Column21d;
		hv_Column21 = HTuple(hv_Column21d);
		infile >> hv_Row22d;
		hv_Row22 = HTuple(hv_Row22d);
		infile >> hv_Column22d;
		hv_Column22 = HTuple(hv_Column22d);
	}
	catch (...)
	{
		hv_Row21 = HTuple(0);
		hv_Column21 = HTuple(0);
		hv_Row22 = HTuple(0);
		hv_Column22 = HTuple(0);
	}
	//}
	UpdateData(FALSE);
}
